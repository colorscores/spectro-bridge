...
const InkConditionsTab = ({ inkId, activeDataMode = 'adapted' }) => {
  ...
  const fetchConditions = useCallback(async () => {
    setLoading(true);
    if (!inkId) {
      console.warn('InkConditionsTab: No inkId provided');
      setConditions([]);
      setLoading(false);
      return;
    }
    try {
      ...
      if (error) {
        console.error('Error fetching ink conditions:', error);
        toast({
          title: 'Error',
      ...
      console.error('Unexpected error fetching ink conditions:', err);
      toast({
        title: 'Error',
  ...
  const handleRowClick = (conditionId) => {
    navigate(`/assets/inks/${inkId}/conditions/${conditionId}`);
  };
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <Button asChild size="sm">
          <Link to={`/assets/inks/${inkId}/conditions/new`}>
                ...
                conditions.map((condition) => (
                  ...
                  >
                    ...
                    <TableCell style={{ width: 'calc(50% - 50px)' }} className="p-4">
                      <span className="font-medium text-gray-800 truncate" title={condition.name}>{condition.name}</span>
                    </TableCell>
...
export default InkConditionsTab;